...
theta_files <- list.files("results",pattern="theta_chain",full.names=TRUE)
theta_ests_all <- NULL
...
write_csv(theta_ests_all, "results/parameter_estimates_all.csv")

## Combine and save attach rate estimates
ar_files <- list.files("results",pattern="attack_rates.csv",full.names=TRUE)
ar_ests_all <- NULL
