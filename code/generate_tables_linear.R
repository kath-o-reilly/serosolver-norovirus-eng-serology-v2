...
theta_files <- list.files("results_linear",pattern="theta_chain",full.names=TRUE)
theta_ests_all <- NULL
...
write_csv(theta_ests_all, "results_linear/parameter_estimates_all.csv")

## Combine and save attach rate estimates
ar_files <- list.files("results_linear",pattern="attack_rates.csv",full.names=TRUE)
ar_ests_all <- NULL
...
write_csv(ar_ests_all, "results_linear/ar_estimates_all.csv")