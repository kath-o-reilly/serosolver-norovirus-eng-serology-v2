...
read_in <- read_csv("inputs/serosolver_norovirus_input.csv")
...
par_tab <- read.csv("inputs/par_tab.csv",stringsAsFactors=FALSE)
...
df <- df %>% dplyr::rename(names=original)
...
ess_labels <- ess_labels %>% left_join(tidy_theta_chain %>% group_by(label) %>% dplyr::summarize(max_val = max(est)))
...
combined_preds_inf_hists <- antibody_data_expanded_long %>% filter(sample_time >= birth) %>% left_join(tmp_states %>% select(-name) %>% dplyr::rename(x=value)) 

## Plot to check
combined_preds_inf_hists %>% group_by(individual,sample_time) %>% dplyr::summarize(prob_infected=sum(x)/n(),mean_titre=mean(value)) %>% filter(individual %in% 1:36) %>%
  ggplot() + 
...
titer_infection_relationship_unmodified <- combined_preds_inf_hists %>% 
  ...
  group_by(samp, value) %>% 
  dplyr::summarize(n_infected=sum(x),N=n()) %>%
...
titer_infection_relationship_unmodified_summary <- titer_infection_relationship_unmodified %>%
  group_by(value) %>% dplyr::summarize(mean=mean(prop_infected),lower=quantile(prop_infected,0.025),upper=quantile(prop_infected,0.975)) %>%
  mutate("Version"="Unmodified")

titer_infection_relationship_modified <- mod %>% 
  mutate(value = round(value,0)) %>% 
  group_by(samp, value) %>% 
  dplyr::summarize(n_infected=sum(x_new),N=n()) %>%
  mutate(prop_infected=n_infected/N) %>% 
  group_by(samp) %>%
  mutate(prop_infected=prop_infected/prop_infected[1])
...
p_titre_relationship_posterior_means <- mod %>% group_by(individual,sample_time) %>% 
  dplyr::summarize(`Probability of infection`=mean(x),`Probability of infection (modified)`=mean(x_new), mean_titre=mean(value)) %>%
...
population_mean_circulating_titre <- antibody_data_expanded_long_all %>% 
  ...
  dplyr::summarize(mean_titre1 = mean(mean_titre),lower=quantile(mean_titre,0.025),upper=quantile(mean_titre,0.975)) %>% filter(sample_time == biomarker_id)
...
population_mean_titres <- antibody_data_expanded_long_all %>% 
  ...
  dplyr::summarize(mean_titre=mean(value)) %>% 
  group_by(sample_time,biomarker_id) %>%
  dplyr::summarize(mean_titre1 = mean(mean_titre),lower=quantile(mean_titre,0.025),upper=quantile(mean_titre,0.975)) %>%
...
p_estimated_model <- plot_estimated_antibody_model(chains$theta_chain,antibody_data=expand_grid(individual=1,sample_time=seq(2001,2012,by=1),biomarker_id=unique(antibody_data$biomarker_id),biomarker_group=unique(antibody_data$biomarker_group)),demographics=NULL,settings=serosolver_settings,by_group=FALSE,nsamp=100,
  ...
  tmp_dat2 <- tmp_dat2 %>% select(biomarker_group,biomarker_id,sample_time,median) %>% dplyr::rename(past_median=median)
  tmp_dat <- left_join(tmp_dat,tmp_dat2) %>% drop_na() %>% dplyr::rename("Current antibody landscape"=median,"Past year antibody landscape"=past_median)
  ...
  p_kinetics_alt <- ggplot(tmp_dat) +
    ...
    labs(tag="A")
  save(p_kinetics_alt, file=paste0("figures/kinetics/estimated_kinetics_",run_name,".rda"))
  ggsave(paste0("figures/kinetics/estimated_kinetics_",run_name,".pdf"),p_kinetics_alt,height=3,width=5,units="in",dpi=300)
  ...
  save(p_kinetics_alt, file=paste0("figures/kinetics/estimated_kinetics_",run_name,"_all.rda"))
  ggsave(paste0("figures/kinetics/estimated_kinetics_",run_name,"_all.pdf"),p_kinetics_alt,height=5,width=6,units="in",dpi=300)
  ...
  save(p_model_schematic, p_schematic_data, file=paste0("figures/model_schematic/model_schematic_",run_name,".rda"))
  
  ggsave(p_model_schematic + p_schematic_data + plot_layout(ncol=2,widths=c(3,1)),
         filename=paste0("figures/model_schematic/model_schematic_",run_name,".pdf"),height=3,width=7,units="in",dpi=300)
  ggsave(p_model_schematic + p_schematic_data + plot_layout(ncol=2,widths=c(3,1)),
         filename=paste0("figures/model_schematic/model_schematic_",run_name,".png"),height=3,width=7,units="in",dpi=300)
} 
...
p_compare_popn_mean_titre_ar <- p_ar$data %>% group_by(time) %>% dplyr::summarize(mean_ar=mean(V1)) %>%
  dplyr::rename(sample_time=time) %>% left_join(population_mean_titres %>% filter(strain == "Matched to \ncirculating virus")) %>%
