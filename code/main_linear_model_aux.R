...
demography_tmp <- antibody_data %>% select(individual,birth) %>% distinct() %>% 
  expand_grid(time=unique(serosolver_settings$antigenic_map$inf_times)) %>% arrange(individual, time) %>% mutate(age = time - birth) %>% 
  mutate(age_group = if_else(age >= 5, "5+",if_else(age <= 0, "0", as.character(age)))) %>% as.data.frame() %>% mutate(age_group = as.numeric(as.factor(age_group))-1)#%>% filter(time >= min(antibody_data$birth))
#antigenic_map <- antigenic_map %>% filter(inf_times >= min(antibody_data$birth))
predicted_titers_all <- get_antibody_level_predictions(chains$theta_chain,chains$inf_chain,
                                                       demographics=demography_tmp,
...
if(analysis == "both"){
  ...
  p_kinetics_alt <- ggplot(tmp_dat) +
    ...
    labs(tag="A")
  save(p_kinetics_alt, file=paste0("figures_linear/kinetics/estimated_kinetics_",run_name,".rda"))
  ggsave(paste0("figures_linear/kinetics/estimated_kinetics_",run_name,".pdf"),p_kinetics_alt,height=3,width=5,units="in",dpi=300)
  ...
  save(p_kinetics_alt, file=paste0("figures_linear/kinetics/estimated_kinetics_",run_name,"_all.rda"))
  ggsave(paste0("figures_linear/kinetics/estimated_kinetics_",run_name,"_all.pdf"),p_kinetics_alt,height=5,width=6,units="in",dpi=300)
  ...
  save(p_model_schematic, p_schematic_data, file=paste0("figures_linear/model_schematic/model_schematic_",run_name,".rda"))
  
  ggsave(p_model_schematic + p_schematic_data + plot_layout(ncol=2,widths=c(3,1)),
         filename=paste0("figures_linear/model_schematic/model_schematic_",run_name,".pdf"),height=3,width=7,units="in",dpi=300)
  ggsave(p_model_schematic + p_schematic_data + plot_layout(ncol=2,widths=c(3,1)),
         filename=paste0("figures_linear/model_schematic/model_schematic_",run_name,".png"),height=3,width=7,units="in",dpi=300)
} 
