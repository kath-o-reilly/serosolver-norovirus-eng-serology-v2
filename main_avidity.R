...
#setwd("~/Documents/local_data/norovirus_test/")

run_name <- "sim_new_1rho"
main_wd <- "local_data/norovirus_test/"
...
mcmc_pars <- c("save_block"=100,"thin"=1,"thin_hist"=5,
               "iterations"=50000,
               "adaptive_period"=30000,
...
use_measurement_offset_parameters <- TRUE
...
antigenic_coords <- data.frame(Strain=sampled_viruses,X=c(0.1,0.5,3,3.5,4),Y=c(0.1,2,1,3,4),inf_times=NA)
antigenic_map <- generate_antigenic_map_flexible(antigenic_coords,year_max=year_max+1,year_min=year_min,spar = 0.1)
ggplot(antigenic_map,aes(x=x_coord,y=y_coord,col=inf_times,label=inf_times)) + geom_line() +
  geom_label() #+
...
antigenic_map <- setup_antigenic_map(antigenic_map,n_biomarker_groups=n_obs_types,
                                     unique_biomarker_groups=1:n_obs_types)[[1]]

write_csv(antigenic_map,paste0(save_wd,"/",run_name,"_antigenic_map.csv"))
...
# load data
antibody_data <- read_csv(paste0(save_wd,"/",run_name,"_titre_data.csv"))
antibody_data <- as.data.frame(antibody_data)

# load antigenic map
antigenic_map <- read_csv(paste0(save_wd,"/",run_name,"_antigenic_map.csv"))

# load par_tab (which ones are switched on/off)
par_tab <- read_csv(file=paste0(save_wd,"/",run_name,"_par_tab.csv"))

## IMPORTANT to set this to a data frame, otherwise can cause some errors later
antibody_data <- as.data.frame(all_simulated_data$antibody_data)
...
prior_func <- function(par_tab){
  ...
  f <- function(pars){
    ...
    p1 <- dlnorm(pars["boost_long"],log(2), 1,log=TRUE) # Looser prior
    p2 <- dlnorm(pars["boost_short"],log(2), 1,log=TRUE) # Looser prior
    p3 <- dbeta(pars["wane_short"], 1, 1,log=TRUE) 
    p4 <- dbeta(pars["antigenic_seniority"],1, 1,log=TRUE)
    p5 <- dbeta(pars["cr_long"],1, 1,log=TRUE)  # Skewed toward low cross-reactivity
    p6 <- dbeta(pars["cr_short"],1, 1,log=TRUE)  # Skewed toward low cross-reactivity
    p7 <- dnorm(pars["obs_sd"],0, 100,log=TRUE) # No change
    p_rhos <- sum(dnorm(pars[rho_pars],0, 25,log=TRUE) )
